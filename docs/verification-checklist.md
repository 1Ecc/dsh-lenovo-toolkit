...
把未验证的 23 工具版本推上 npm，风险比保持旧版（`0.1.1`，只有电池）更大。
...
| # | 工具 | 怎么测 | 通过标准 |
|---|---|---|---|
| B-18 | `battery_warranty_lookup` | 告知会把 SN 发到联想；未确认和确认后各调一次 | 未确认被拒；确认后能区分电池在保、保外或不确定，不把整机延保自动当电池在保 |
| B-19 | `battery_part_price_lookup` | 同上 | 只返回电池项；没有公示价时 `available=false`，不自行估价 |
| B-20 | `battery_service_stores` | 用城市/位置查询 | 返回门店并明确距离是否估算；定位失败改问城市，不循环重试 |
| B-21 | `battery_appointment_start` | 用户本人登录后建立预约会话 | 不代填账号密码；返回会话引用和绑定设备，不在模型上下文暴露 token |
| B-22 | `battery_appointment_options` | 选门店并查时段 | 当天和约满时段不可选；门店、时段均来自实时返回 |
| B-23 | `battery_appointment_submit` | 先不确认，再用测试单明确确认 | 未确认必须拒绝；真实提交只做一次，超时或重复预约不自动重发 |
